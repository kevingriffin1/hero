"""
Must have a DynamoDB table with a stream enabled.  The stream must filter:
...
def lambda_handler(event, context):
    ...
    for key in collections.keys():
        ...
        insert_chunks = [
            list_data[x : x + max_batch_size]
            for x in range(0, len(list_data), max_batch_size)
        ]
        for records in insert_chunks:
            response = sqs.send_message_batch(QueueUrl=key, Entries=records)
